      ...
      DO IX=2,NX-1
         DO IY=2,NY-1
            ...
            UYY(IY,IX) = UHS(IY,IX)      ! SAVE ONE MORE COPY OF DT * D_YY TO BE USED IN THE 2ND STEP
      ...
      DO IY = 2,NY-1
         DO IX = 2,NX-1
            UH(IY,IX) = UHS(IY,IX) / BETA(IY,IX) - UYY(IY,IX)
         END DO
      END DO
      ...
      CALL SETBC_ADID1(T+DT,0.0D0,UH)
      ...
      DO IX = 2,NX-1
         ...
         CALL D_YY(IX,DT,UH,A,B,C,R)   ! CONVERT LHS OPERATOR (1/BETA - DT D_YY) TO A TRI-DIAGONAL MATRIX
                                       ! AND STORE IT IN A, B, AND C WITH CORRESPONDING RHS R
