...
!      END FUNCTION IFBETA2
      ...
      ELSEIF ((TMP.GT.Y1) .AND. (TMP.LT.Y2)) THEN
         Y = TMP
      ELSEIF ((-TMP.GT.Y2) .AND. (-TMP.LT.Y1)) THEN
         Y = -TMP
      ELSE
      ...
      DATA%JUMP(3) = -VK*SIN(THETA)*COS(VK*XINF)*COS(VK*YINF)&
                     -VK*COS(THETA)*SIN(VK*XINF)*SIN(VK*YINF)
