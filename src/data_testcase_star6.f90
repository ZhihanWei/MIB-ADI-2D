      ...
      !    F=2*K^2*BETA1*SIN(KX)*COS(KY)-SIN(T)     INSIDE
      !     =2*K^2*BETA2*COS(KX)*SIN(KY)-SIN(T)     OUTSIDE
      !
      ...
      DO IY = 1,NY
         DO IX = 1,NX
            IF ( SETS(XI(IX),YI(IY)) .GT. TOL_SETUP ) THEN
               U(IY,IX) = COS( VK*XI(IX) ) * SIN( VK*YI(IY) ) + CT !OUTSIDE
            ELSE
               U(IY,IX) = SIN( VK*XI(IX) ) * COS( VK*YI(IY) ) + CT !INSIDE
            END IF
         END DO
      END DO
      ...
      DO IY = 1,NY
         DO IX = 1,NX
            IF ( SETS(XI(IX),YI(IY)) .GT. TOL_SETUP ) THEN  !OUTSIDE
               SRC(IY,IX) = 2.0D0 * BETA(IY,IX) * VK**2 * COS(VK*XI(IX)) * SIN(VK*YI(IY)) - SIN(T)
            ELSE                                          !INSIDE
               SRC(IY,IX) = 2.0D0 * BETA(IY,IX) * VK**2 * SIN(VK*XI(IX)) * COS(VK*YI(IY)) - SIN(T)
            END IF
         END DO
      END DO
      ...
      !       = COS(KX)*SIN(KY) + ( COS(T) + COS(T+DT) )/2
      !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      DATA%JUMP(2) = ( SIN(VK*XINF) * SIN(VK*YINF) * ( BETA1 * SIN(THETA) - BETA2 * COS(THETA) ) + &
                     & COS(VK*XINF) * COS(VK*YINF) * ( BETA2 * SIN(THETA) - BETA1 * COS(THETA) ) ) * VK
      ...
      DATA%JUMP(3) = ( COS(VK*XINF) * COS(VK*YINF) + SIN(VK*XINF) * SIN(VK*YINF) ) * VK * ( COS(THETA) + SIN(THETA) )
      ...
      DATA%JUMP(4) = -VK * ( BETA2 * SIN(VK*XINF) * SIN(VK*YINF) + BETA1 * COS(VK*XINF) * COS(VK*YINF) )
      DATA%JUMP(5) = -VK * ( BETA2 * SIN(VK*XINF) * SIN(VK*YINF) + BETA1 * COS(VK*XINF) * COS(VK*YINF) )
      ...
      DATA%JUMP(4) = VK * ( BETA2 * COS(VK*XINF) * COS(VK*YINF) + BETA1 * SIN(VK*XINF) * SIN(VK*YINF) )
      DATA%JUMP(5) = VK * ( BETA2 * COS(VK*XINF) * COS(VK*YINF) + BETA1 * SIN(VK*XINF) * SIN(VK*YINF) )
      ...
      FUNCTION UTAU_MINUS(X,Y,T,THETA)
         ...
         UTAU_MINUS = -VK * ( SIN(THETA) * COS(VK*X) * COS(VK*Y) + COS(THETA) * SIN(VK*X) * SIN(VK*Y) )
      ...
      FUNCTION UTAU_PLUS(X,Y,T,THETA)
         ...
         UTAU_PLUS = VK * ( SIN(THETA) * SIN(VK*X) * SIN(VK*Y) + COS(THETA) * COS(VK*X) * COS(VK*Y) )
